...
async function createFavoriteIcon(postId, isActive) {
  try {
    ...
    favorite.src = isActive ? "images/coeur_favoris.png" : "images/coeur_vide.png";
    ...
    favorite.onclick = async function () {
      ...
      if (success) {
        favorite.src = newFavoriteStatus ? "images/coeur_favoris.png" : "images/coeur_vide.png";
...
async function DisplayRecettes() {
  try {
    const response = await fetch("data_instagram/your_instagram_activity/content/posts_1.json");
    if (!response.ok) {
      throw new Error("Network response was not ok");
    }
    const data = await response.json();
...
creationDom(index,post,media)
    ...
    if (!memFavoris) {
      const postElement = document.createElement('div');
        ...
        // Titre
        // Divise la chaîne en fonction du caractère "#"
        let mediaTitleSplit = "Oups Aucun favoris n'a été sélectionné " + String.fromCodePoint(0x1F609);
        const title = document.createElement('div');
        title.className = 'title';
        title.textContent = decodeUTF8(mediaTitleSplit);
        title.style.textAlign = "center"
